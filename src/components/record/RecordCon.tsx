                    ...
                    <>
                        {(logsType.current === 'tatakai' || at.logs_type === 'tatakai') && at.skillId !== 'default' && (
                            <span className="text-sm">
                                ...
                                {/* 施加BUFF */}
                                {at.buffs?.length && <span>{at.buffs.map((buff) => <span>&#160;-&gt; {battleManager.current.BuffManage.buff_map[buff.id].isDebuff ? "DEBUFF" : "BUFF"} -&gt;&#160;{battleManager.current.BuffManage.buff_map[buff.id]?.name}</span>)}</span>}
                                {/* 击杀 */}
