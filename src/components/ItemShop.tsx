...
import { items_medic } from '../common/items/data/medic';
import { z } from 'zod';
...
const ItemShop: React.FC<ItemShopProps> = ({ data = [], playerCurrency = 0, onPurchase, prop_id = "id" }) => {
  ...
  return (
    <div className="p-3">
      <h2 className="text-xl font-bold mb-2">医疗物品商店</h2>
      <div className="mb-2">
      ...
      <div className="grid grid-cols-10 gap-2">
        {data.map((item) => {
          const quantity = getQuantity(item[prop_id]);
              ...
              <div className="mt-2">
                ...
                <div className="flex items-center space-x-1 mt-1">
                  <InputNumber
                    ...
                    min="1"
                    value={quantity}
                    onChange={(e) => handleQuantityChange(item[prop_id], parseInt(e.target.value) || 1)}
                  ...
                  <button
                    ...
                    onClick={() => canAfford && onPurchase(item[prop_id], quantity)}
                    disabled={!canAfford}
                  >
                    购买
                  </button>
